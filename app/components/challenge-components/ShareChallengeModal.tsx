      ...
      <div className="relative z-10 max-h-[96vh] w-full max-w-5xl overflow-y-auto border-2 border-black bg-[#fff8f4] p-4 sm:p-6">
        ...
        <div className="grid items-start gap-5 md:grid-cols-[minmax(0,1.12fr)_minmax(250px,.88fr)]">
          ...
          <div className="space-y-5">
            ...
            {!winner && <fieldset><legend className="mb-2 text-xs font-black uppercase tracking-[.14em] text-[#5f5048]">2. Pick a look</legend><div className="grid grid-cols-2 gap-2"><button type="button" onClick={() => setTheme("arena")} aria-pressed={theme === "arena"} className={`flex cursor-pointer items-center justify-center gap-2 border-2 border-black px-3 py-3 text-sm font-black transition ${theme === "arena" ? "bg-[#1e1720] text-white" : "bg-white"}`}><Swords className="h-4 w-4" />Arena</button><button type="button" onClick={() => setTheme("poster")} aria-pressed={theme === "poster"} className={`flex cursor-pointer items-center justify-center gap-2 border-2 border-black px-3 py-3 text-sm font-black transition ${theme === "poster" ? "bg-[#e85a2d] text-white" : "bg-white"}`}><Sparkles className="h-4 w-4" />Hype</button></div></fieldset>}
