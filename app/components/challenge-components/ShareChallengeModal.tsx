...
import { Check, Copy, Download, Link2, Share2, Sparkles, Swords, X } from "lucide-react";
...
function loadImage(src: string) {
  return new Promise<HTMLImageElement>((resolve) => {
    const image = new window.Image(); image.onload = () => resolve(image); image.onerror = () => resolve(image); image.src = src;
  });
}
...
export function ShareChallengeModal({ challenge, isOpen, onClose }: ShareChallengeModalProps) {
  ...
  const drawCard = useCallback(async () => {
    ...
    const [mascot, logo] = await Promise.all([loadImage("/welcome/rekto-mascot-arena.jpg"), loadImage("/rektologo.png")]);
  ...
  }, [challenge, side, statement, stats.participants, stats.pool, theme, username]);
  ...
  const downloadImage = async () => { const blob = await canvasBlob(); const url = URL.createObjectURL(blob); const link = document.createElement("a"); link.href = url; link.download = `rektofun-challenge-${challenge.id}.png`; link.click(); URL.revokeObjectURL(url); };
  const share = async () => {
    const text = `${side === "TEAM_A" ? "I back this call" : "I’m calling rekto"}: ${stripUsdcQuote(challenge.statement || challenge.title)}`;
    try {
    ...
    <div className="fixed inset-0 z-[10100] flex items-center justify-center p-3 sm:p-5" role="dialog" aria-modal="true" aria-labelledby="share-challenge-title">
      ...
      <div className="relative z-10 max-h-[96vh] w-full max-w-5xl overflow-y-auto border-2 border-black bg-[#fff8f4] p-4 sm:p-6">
        <div className="mb-5 flex items-start justify-between gap-4">
          <div><div className="mb-1 flex items-center gap-2 text-[#7b3dc1]"><Sparkles className="h-4 w-4" /><span className="text-[11px] font-black uppercase tracking-[.18em]">Share studio</span></div><h2 id="share-challenge-title" className="text-2xl font-black text-gray-950">Make your call loud.</h2><p className="text-sm font-semibold text-[#786a61]">Choose your stance, style the card, then send it into the arena.</p></div>
