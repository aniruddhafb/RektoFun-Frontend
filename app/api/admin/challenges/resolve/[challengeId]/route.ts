...
export async function POST(
  ...
  if (!cronApiKey) {
    return NextResponse.json({ error: "CRON_API_KEY is not configured" }, { status: 500 });
  }
  ...
  try {
    const response = await fetch(`${apiBase}/challenges/cron/resolve/${challengeId}`, {
      method: "POST",
      headers: {
    ...
    console.error(`[api/admin/challenges/resolve/${challengeId}] error:`, error);
    return NextResponse.json(
